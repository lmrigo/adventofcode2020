...
var part2 = function () {
  ...
  for (var i = 0; i < input.length; i++) {
    ...
    // console.log(grid)
    // console.log(x1,x2,y1,y2,z1,z2,w1,w2)

    var calcAdj = function(ci,cj,ck,cl) {
      var adj = ''
       for (var ai = ci-1; ai <= ci+1; ai++) {
        if (grid[ai] === undefined) {
          grid[ai] = []
        ...
        for (var aj = cj-1; aj <= cj+1; aj++) {
          ...
          for (var ak = ck-1; ak <= ck+1; ak++) {
            ...
            for (var al = cl-1; al <= cl+1; al++) {
              if ((ai === ci) && (aj === cj) && (ak === ck) && (al === cl)) {
                continue // skip the source position
              }
              if (grid[ai][aj][ak][al] !== undefined) {
                adj += grid[ai][aj][ak][al]
              } else {
                adj += ''
              }
            }
    ...
    var cycle = function (i1,i2,j1,j2,k1,k2,l1,l2) {
      var newGrid = []
      for (var si = i1; si < i2; si++) {
        newGrid[si] = []
        if (grid[si] === undefined) {
        ...
        for (var sj = j1; sj < j2; sj++) {
          ...
          for (var sk = k1; sk < k2; sk++) {
            newGrid[si][sj][sk] = []
            if (grid[si][sj][sk] === undefined) {
              grid[si][sj][sk] = []
            }
            for (var sl = l1; sl < l2; sl++) {
              // console.log(si,sj,sk,sl)
              var adjacency = calcAdj(si,sj,sk,sl)
              var activeNeighbours = (adjacency.match(/#/g) || []).length
              // If a cube is active
              if (grid[si][sj][sk][sl] === '#') {
                //  and exactly 2 or 3 of its neighbors are also active,
    ...
    var c = 0
    while (c++ < totalCycles) {
      grid = cycle(x1-c,x2+c+1,y1-c,y2+c+1,z1-c,z2+c+1,w1-c,w2+c+1)
    }
    // console.log(grid)
    ...
    for (var gi = x1-c+1; gi < grid.length; gi++) {
      for (var gj = y1-c+1; gj < grid[gi].length; gj++) {
        for (var gk = z1-c+1; gk < grid[gi][gj].length; gk++) {
          for (var gl = w1-c+1; gl < grid[gi][gj][gk].length; gl++) {
            sum += (grid[gi][gj][gk][gl] === '#' ? 1 : 0)
          }
        }
      }
    }

    var result = sum    // console.log(result)
